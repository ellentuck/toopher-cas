...
public class LevelOfAssuranceManager {
    ...
    private static final Logger logger = Logger.getLogger("com.toopher.integrations.cas.web.flow.LevelOfAssuranceManager");
    ...
    public Principal getUserPrincipalForTGT(String ticketGrantingTicketId) {
        ...
        TicketGrantingTicket ticketGrantingTicket = 
          (TicketGrantingTicket)ticketRegistry.getTicket(ticketGrantingTicketId, TicketGrantingTicket.class);
    ...
    public Long getRequiredLevelOfAssurance(Principal userPrincipal, Service service) {
    	String serviceId = service.getId();
    	return getRequiredLevelOfAssurance(userPrincipal, serviceId);
    }
    
    public String evaluateEstablishedLevelOfAssurance(String TGTid, Service service) {
    	logger.debug("Will evaluate LOA for TGT " + TGTid + " and service " + service.getId());
    	
    	Long establishedLoa = getEstablishedTGTLevelOfAssurance(TGTid);
    	logger.debug("Established LOA for " + TGTid + ": " + establishedLoa);
    	
    	Principal userPrincipal = getUserPrincipalForTGT(TGTid);
    	Long requiredLoa = getRequiredLevelOfAssurance(userPrincipal,service);
    	logger.debug("Required LOA for " + TGTid + ": " + requiredLoa);
    	
        return evaluateEstablishedLevelOfAssurance(establishedLoa,requiredLoa);  
    }
    
    
    public Long getRequiredLevelOfAssurance(Principal userPrincipal, String serviceId) {
    	logger.debug("getRequiredLevelOfAssurance() for " + userPrincipal.getId() + " : " + serviceId);
        LevelOfAssurance serviceLoa = null;
        LevelOfAssurance userLoa = null;
        if (serviceLoaResolver != null) {
            if (serviceId != null) {
                serviceLoa = serviceLoaResolver.getLoa(new SimplePrincipal(serviceId));
            }
        }
        ...
        logger.debug("service loa: " + serviceLoa.asString() + " user loa: " + userLoa.asString());
    ...
    public boolean getToopherRequired(Principal userPrincipal, Service service) {
    	String serviceId = service.getId();
        Long requiredLoaVal = getRequiredLevelOfAssurance(userPrincipal, serviceId);
